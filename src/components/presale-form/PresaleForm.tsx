...
const PresaleForm = () => {
  ...
  useEffect(() => {
    ...
    const shouldPollVerification = !isVerified;
    ...
    let verifyInt: ReturnType<typeof setInterval> | undefined;
    if (shouldPollVerification) {
      verifyInt = setInterval(() => checkVerificationStatus(address), 5000);
    }
  ...
  //   if (!amount || amount <= 0 || Number.isNaN(amount)) return alert("Enter amount");
  //   if (!isVerified) return alert("Complete verification");
  //   if (!selectedCurrencyData.isActive) return alert(`${selectedCurrencyData.symbol} disabled`);
  ...
  //     // Prefer explicit function over raw call to avoid decode errors
  //     const authorizer = new Contract(authAddr, AUTHORIZER_ABI, provider);
  //     // Use getNonce (view) which is part of the ABI; fallback to 'nonces' if needed
  //     let nonce: any;
  //     try {
  //       nonce = await authorizer.getNonce(address);
  //     } catch {
  //       // some versions expose mapping accessor 'nonces(address)'
  //       nonce = await authorizer.nonces(address);
  //     }

  //     const usdAmountValue = Number(amountInput) * selectedCurrencyData.priceUsd;
  //     const apiUrl = import.meta.env.VITE_API_URL || "https://dynastical-xzavier-unsanguinarily.ngrok-free.dev";
  ...
  //     const { voucher, signature } = data;
  //     const presaleContract = new Contract(PRESALE_CONTRACT_ADDRESS, PRESALE_ABI, signer);
  //     const voucherStruct = [voucher.buyer, voucher.beneficiary, voucher.paymentToken, voucher.usdLimit, voucher.nonce, voucher.deadline, voucher.presale];

  //     let tx;
  //     if (isNative) {
  //       const ethAmount = parseEther(amountInput);
  //       tx = await presaleContract.buyWithNativeVoucher(address, voucherStruct, signature, { value: ethAmount });
  //     } else {
  //       const tokenContractRead = new Contract(paymentToken, ERC20_ABI, provider);
  //       const tokenAmount = parseUnits(amountInput, selectedCurrencyData.decimals);
  ...
  //         await approveTx.wait();
  //       }
  //       tx = await presaleContract.buyWithTokenVoucher(paymentToken, tokenAmount, address, voucherStruct, signature);
  //     }
  ...
  //   } catch (err: any) {
  //     alert(`Failed: ${err.response?.data?.error || err.reason || err.message}`);
  //   } finally {
  //     setLoading(false);
  //   }
  // };
